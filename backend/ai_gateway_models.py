...
class User(Base):
    __tablename__ = "ai_gateway_users" # Renamed to avoid conflict with potential future Keycloak user table
    id = Column(Integer, primary_key=True)
    organization_id = Column(Integer, ForeignKey("organizations.id"), nullable=False)
    keycloak_user_id = Column(String, unique=True, nullable=False, index=True) # Link to Keycloak user 'sub'
...
class UsageLog(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("ai_gateway_users.id"), nullable=False) # Changed to ai_gateway_users.id
    organization_id = Column(Integer, ForeignKey("organizations.id"), nullable=False)
...
class Alert(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("ai_gateway_users.id"), nullable=True) # Changed to ai_gateway_users.id
